...
- AutoWriter 在真实模型上的长期长文质量稳定性、保留率、人物一致性和世界观一致性；本轮已完成一条固定短样例烟测、一次 3 章连续 smoke 和一次 3 题材同题对照，但还没有覆盖长期多模型稳定性和人工保留率评分。
- 与普通 AI 聊天窗口相比，真实作者总耗时、修改成本和上下文整理成本是否明显下降；本轮只完成了一次自动化同题对照，证明平均少手工拼接约 511 个上下文字符，但 AutoWriter 墙钟生成耗时更长。
- Windows/macOS/Linux 安装包在干净机器上的首次启动、升级和卸载体验；本轮只验证了本机 Windows unpacked 目录版进程级启动。
- 长时间写作、异常断电、数据库损坏、复杂项目迁移等压力场景。

## 下一轮应该验证

1. 扩展 3 题材同题对照到更多题材和至少 2 个模型，保留大纲遵循、人物一致、世界观一致、前文承接、可读性、可编辑性、排版评分。
2. 给真实模型长篇连续 smoke 和 proof suite 增加人工评分：每章可保留比例、需要修改的明显错误数、严重 OOC 数、世界观违例数。
3. 与普通 AI 聊天窗口做真人同题对比：记录作者准备上下文耗时、明显设定错误、作者修改耗时、可保留字数比例。
