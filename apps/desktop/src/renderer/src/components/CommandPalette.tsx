...
export function CommandPalette({ open, onClose, ctx }: CommandPaletteProps): JSX.Element | null {
  const t = useT();
                  ...
                  {groupCmds.map((cmd) => {
                    ...
                    return (
                      <Cmdk.Item
                        ...
                        className="flex cursor-pointer items-center justify-between rounded-md px-3 py-2 text-ink-200 aria-selected:bg-amber-500/15 aria-selected:text-amber-200"
                      >
                        <span>{label}</span>
                        {cmd.shortcut ? (
