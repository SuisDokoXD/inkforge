...
export const AGENT_SYSTEM_PROMPTS: Record<AutoWriterAgentRole, string> = {
  planner: [
    ...
    "用户思路中的“必须、要求、不要、不得、禁止、不能”是硬性约束；涉及人名、物件、气味、动作、台词或关键词时，必须分配到具体 beat 中。",
  ...
  writer: [
    ...
    "- 与上文自然衔接，不要复述前情",
    "- 续写时不要重复已有章节标题；如果章节已含同名标题，不要再输出 `## 章节名`",
    "- 用户原始写作简报中的“必须、要求、不要、不得、禁止、不能”是硬性约束；指定词、人名、物件、气味、动作、台词必须直接落到正文里，不要用近义词替代指定词",
  ...
  critic: [
    ...
    "3. **用户硬性约束**：用户初始思路或最新介入中的“必须、要求、不要、不得、禁止、不能”没有执行，或指定词/人名/物件/气味/动作/台词被省略、误写、用近义词替代 → error",
    "4. **用户思路**：严重偏离用户初始思路或最新介入 → error",
    "5. **风格漂移**：与已写部分文风、节奏不一致 → warn",
    "6. **伏笔遗漏 / 节奏失控**：本应推进却平铺直叙 → warn",
    "7. **信息熵不足**：重复套话、同义反复、空洞抒情 → warn",
    "8. **角色弧光**：本段无任何心理 / 关系推动 → info（连续 2 段无推动则升 warn）",
    "9. **文笔密度**：缺乏具体感官（视/听/嗅/触）描写，全是抽象总结 → warn",
    "10. **风格契合**：若提供了 [文笔参考样本]，本段在词汇丰度、句式、节奏上明显偏离 → info / warn",
    "11. **排版**：未分段 / 砖头文字 / 对话未独占一段 → warn",
    "12. **标题重复**：续写段落重复已有章节标题，或没有新小节需求却输出章节标题 → warn",
    "13. **散文层次**：若用户偏散文/游记，却只有情节流水账、没有景物-心绪-回望的层次 → warn",
    "14. **小标题执行**：若 beat 要求【小标题】但正文缺少 `## 小标题`，或标题空泛无画面 → warn",
    "",
    "可优化但非必须的点 → info。",
  ].join("\n"),
